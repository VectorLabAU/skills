...
Eval protocol for the VectorLab UI/UX skills catalog (**17 skills**: 1 setup + 16 domain). Success means an agent can **install**, **configure**, and **apply** the skills — not only that the markdown files exist.
...
| # | Check | Pass criteria |
| --- | --- | --- |
| 0.1 | Skill count | Exactly **17** folders with `SKILL.md` under `skills/` (excluding `skills/config/`) |
...
| 0.7 | Plugin | `.claude-plugin/plugin.json` lists all **17** paths; `marketplace.json` source is `./` |
...
| Expect | Pass if |
| --- | --- |
| Skill picker | All **17** skills offered |
| Setup included | `setup-ui-ux-skills` is selectable / selected |
| Install target | Files land in agent skill dir (e.g. Cursor `.agents/skills`) |
...
If `claude` CLI is available, install from `.claude-plugin/marketplace.json`. Expect the same 17 skills; setup remains user-invoked only.
...
1. Explore repo first (`.ux-profile.md`, `AGENTS.md` / `CLAUDE.md`, brand/tokens, **icon library signals**)
...
```text
Finish the UI. Run the pre-flight taste audit from the UI/UX skills README.
```

### Pass if

- Transcript / tool reads show `.ux-profile.md` and matching domain skills were opened
...
### Pre-flight taste audit (README)

After scoring skills, also verify:
